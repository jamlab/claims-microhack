# Challenge 6 - Optionally Host Secure Actions in Azure Functions
...
> [!NOTE]
> Challenge 6 is optional. Challenge 5 completes the local secured claims workflow.
...
- Complete [Challenge 5](./challenge-05.md)
...
- `FOUNDRY_PROJECT_ENDPOINT`: the same project endpoint used in Challenges 2-5
- `FOUNDRY_MODEL`: the same model deployment used in Challenges 2-5
...
like the result from Challenge 4 and a claimant-authored message:
...
decision must remain unchanged. A strict payout sink blocks after any untrusted
claimant content enters the agent context, even when that text is benign. This
conservative behavior is expected and matches Challenge 5. The audit entry uses
`type: untrusted_context` and identifies `approve_payout` in its `function` field.
...
that the audit log records `approve_payout` with `type: untrusted_context`. Then
run the denied-workflow exfiltration request in `demo.http`. Confirm that no SSN
or prior-claim history appears in the response. The model may reject the malicious
request without calling the private-record tool. If it attempts the protected tool
chain, confirm that the audit records `notify_claimant` with
...
The local and deployed hosts expose `POST /api/claims/actions`. A valid request returns
the original trusted workflow status, a public response, and a FIDES audit log. Each
audit entry identifies the blocked tool in `function`, the violation category in
`type`, and the policy explanation in `reason`. Payout-injection attempts do not
produce the requested unauthorized payment, and exfiltration attempts do not expose
private policyholder data.

## Validation checklist

- The local host exposes `POST /api/claims/actions`
- The request contains a completed Challenge 4 `workflow_result`
...
Challenges 2-5 form the complete local microhack. By finishing this optional
